...
3. **Bugzilla analytics.** The API also exposes `/api/v1/analytics/search` with `scope=bugs`.
   The `find_bugs_in_tests(package)` tool queries this endpoint to return Bugzilla bugs
   mentioning a package -- cached 24h in the `testcatalog_bugs` table (migration 006).
...
Shipped alongside `rpm-mcp doctor`. Calls `GET /api/v1/analytics/search?q=<package>&scope=bugs`
and returns Bugzilla bugs mentioning the package, formatted as:

```
Bugs for systemd (10 results):
  [RESOLVED  ] bsc#1234567 -- systemd-maintainers@suse.de
    [Basesystem, Normal] systemd fails to start on some BIOS configurations
```

Results are cached 24h in `testcatalog_bugs (id, package_id, bug_id, summary, status, severity,
component, assigned_to, resolution)`. The stale-data WARNING banner fires on API failure
exactly as it does for changelog sources.
