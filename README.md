...
> **Advisory tool — not a source of truth.** Output is generated by an LLM reasoning over cached package metadata. Results are non-deterministic, may be incomplete or outdated, and must always be verified against authoritative sources (OBS, Bugzilla, CVE databases) before acting on them. This tool is designed to help engineers explore and triage — not to replace human judgement.
